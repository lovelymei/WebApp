...
{
    [TestFixture]
    class PerformersInSqlRepositoryTests
    {
        ...
        [Test]
        public async Task GetAllPerformerSongs_Id_ListSongsDto()
        {
            //Arrange;
            var songs = GetSongs();
            var performer = new Performer();
            performer.Songs = songs;

            var db = TestsRepositoryService.GetClearDataBase();
            db.AddEntity(performer);

            var repository = new PerformersInSQLRepository(db);
            var expected = GetSongsDto();

            //Act
            var actualIEnumerable = await repository.GetAllPerformerSongs(performer.EntityId);
        ...
        [Test]
        public async Task GetAllPerformerAlbums_Id_ListAlbumsDto()
        {
            //Arrange;
            ...
            //Act
            var actualIEnumerable = await repository.GetAllPerformerAlbums(performer.EntityId);
        ...
        public async Task AttachSongToPerformer_SongId_ResponceCode()
        {
            ...
            //Act
            var actual = await repository.AttachSong(song.EntityId, performer.EntityId);
        ...
        public async Task AttachAlbumToPerformer_AlbumId_ResponceCode()
        {
            ...
            //Act
            var actual = await repository.AttachAlbum(album.EntityId, performer.EntityId);
